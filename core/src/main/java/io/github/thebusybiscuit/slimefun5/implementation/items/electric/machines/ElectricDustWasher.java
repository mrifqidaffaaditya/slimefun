    ...
    protected MachineRecipe findNextRecipe(BlockMenu menu) {
        for (int slot : getInputSlots()) {
            ...
            if (SlimefunUtils.isItemSimilar(input, SlimefunItems.SIFTED_ORE.item(), true, false)) {
                ...
                recipe = new MachineRecipe(Math.max(1, 4 / getSpeed()), new ItemStack[] { SlimefunItems.SIFTED_ORE.item() }, new ItemStack[] { oreWasher.getRandomDust() });

                if (!legacyMode || fitsOutput(menu, recipe.getOutput())) {
                    menu.consumeItem(slot);
                    return recipe;
                }
            } else if (SlimefunUtils.isItemSimilar(input, SlimefunItems.PULVERIZED_ORE.item(), true)) {
                recipe = new MachineRecipe(Math.max(1, 4 / getSpeed()), new ItemStack[] { SlimefunItems.PULVERIZED_ORE.item() }, new ItemStack[] { SlimefunItems.PURE_ORE_CLUSTER.item() });
            } else if (SlimefunUtils.isItemSimilar(input, new ItemStack(Material.SAND), true)) {
                recipe = new MachineRecipe(Math.max(1, 4 / getSpeed()), new ItemStack[] { new ItemStack(Material.SAND) }, new ItemStack[] { SlimefunItems.SALT.item() });
            }

            if (recipe != null && fitsOutput(menu, recipe.getOutput())) {
                menu.consumeItem(slot);
                return recipe;
            }
        }
